...
/***********************************对比分析视图*************************************/
class ContrastAnalysisView extends React.Component {
	constructor(props){
		super(props);
	...
	$('.react-bs-container-body').mCustomScrollbar({
	    mouseWheel: {
	        preventDefault: true
	    },
	    theme:"dark-2",
  	...
  	onProgress = (data) =>
	{
		$('.react-bs-container-body').mCustomScrollbar("scrollTo","-=38");
		 		 this.setState({selectRow:this.state.selectRow+1});
	}
  ...
  console.log("selected: " + isSelected)
	...
	render() {
		return (
			<div id='testDiv'  style={{overflow: 'hidden', maxWidth: 935}} className={styles.mainBox}>
				<h3>{this.props.params.id} </h3>
				<DgnViewer  height='450px' width='900px' />
				<row >
					<PlayBar onProgress={this.onProgress} playing="pause" progress={1} data={playBarData} loop={true} duration={2000} />
				</row>
				<row  gutter={24} >
					<Form >
						<Col sm={5}>
							<FormItem label="分项列表" labelCol={{ span: 9 }} wrapperCol={{ span:15 }} >
								<Select defaultValue="1" dropdownMatchSelectWidth="false">
									...
									<Option title='近坝岸坡处理工程' value="2">近坝岸坡处理工程</Option>
								</Select>
							</FormItem>
						</Col>
						<Col sm={4}>
...
{/*		         <Col sm={24} offset={0} >
				...
				<row >
					<Col lg={24}  style={{marginTop:'-20px'}}>
						<BootstrapTable data={data} selectRow={this.selectRowProp()} height="200px">
							...
							<TableHeaderColumn dataField="divide" width="100">施工包划分1</TableHeaderColumn>
						</BootstrapTable>
					</Col>
				</row>
			</div>
...
export default ContrastAnalysisView;
